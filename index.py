...
# def create_meaningful_chunks(paragraphs):
#     chunks = []
#     chunk = []
#     chunk_size = 500  # Set desired chunk size based on word count
    
#     for para in paragraphs:
#         words = word_tokenize(para)
#         filtered_words = remove_stopwords_and_punctuation(words)
#         if len(chunk) + len(filtered_words) > chunk_size:
#             chunks.append(' '.join(chunk))
#             chunk = filtered_words
#         else:
#             chunk.extend(filtered_words)
...
#             f.write(f"Chunk {i}:\n{chunk}\n\n")
#     print(f"Chunks saved to file: {file_path}")
...
#     chunks_file_path = os.path.join(base_output_path, f"{os.path.basename(pdf_path).replace('.pdf', '')}_chunks.txt")
...
#     paragraphs = segment_text_into_paragraphs(all_text)
#     meaningful_chunks = create_meaningful_chunks(paragraphs)
#     save_chunks_to_file(meaningful_chunks, chunks_file_path)
...
try:
    ...
    print("coreferee is not installed. Please install it using 'pip install coreferee'.")
...
def extract_relationships(text):
    ...
    if doc._.has_coref_chains:
        for chain in doc._.coref_chains:
            main = doc[chain.main].text
            for mention in chain.mentions:
                if mention != main:
                    relationships.append((main, "is", doc[mention].text))
    
    return relationships

def save_relationships_to_file(relationships, file_path):
    with open(file_path, 'w', encoding='utf-8') as f:
        for subject, verb, obj in relationships:
            f.write(f"{subject} {verb} {obj}\n")
    print(f"Relationships saved to file: {file_path}")
...
def process_pdf(pdf_path, base_output_path):
    ...
    frequency_file_path = os.path.join(base_output_path, f"{os.path.basename(pdf_path).replace('.pdf', '')}_word_frequency.txt")
    page_info_path = os.path.join(base_output_path, f"{os.path.basename(pdf_path).replace('.pdf', '')}_page_info.txt")
    relationships_file_path = os.path.join(base_output_path, f"{os.path.basename(pdf_path).replace('.pdf', '')}_relationships.txt")
    ...
    relationships = extract_relationships(' '.join(all_filtered_tokens))
    save_relationships_to_file(relationships, relationships_file_path)
